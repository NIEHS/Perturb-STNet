...
plotTree <- function(mst,
  ...
  # cols -> Colors when PlotVar is discrete
...
GetTreeVariableGenesDynamics.INLA <- function(
  ...
  ko_param <- foreach(i=seq_len(length(names_gene)),.errorhandling = "pass",
    ...
    # Get Signal to Noise ratio
    sSpat =  var(m1$summary.random$s$mean)  #1/m1$summary.hyperpar["Precision for s",1]
...
FindNullDistribution<- function(Control, Perturbed,monteCarloDraws){
 ...
 nam =  intersect(names( Control$SNR[3,]), names(Perturbed$SNR[3,nam]))
...
GetNullDistParameters <- function(mst,mst_denoded,Dist,Location_link,
                                  Scale_link,
                                  nu_link=NULL,
                                  tau_link=NULL){
  n=50
