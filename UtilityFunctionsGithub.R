...
Hcluster <- function(DataTocluster,thresholdGini=0.2,k,ClusterName){
  ...
  cluster  <- KMeans_rcpp(as.matrix(DataTocluster), clusters = k, num_init = 1,
                          initializer = "kmeans++")
...
SpatioTemporalEstimation <- function(mst,
                                     ExprsData,
  ...
  mst_grid = mst
  Data_sub= ExprsData
  days = Data_sub[,TemporalCol]
  
  
  Model_inla = gamlss_to_inla_family <- function(family) {
    switch(family,
           
           # Continuous distributions
           "NO"  = "gaussian",        # Normal
           "LOGNO" = "lognormal",     # Log-normal
           "GA"  = "gamma",           # Gamma
           "IG"  = "invgamma",        # Inverse Gamma (if using priors)
           "WEI" = "weibull",         # Weibull
           "BCT" = "gaussian",        # Approximate as gaussian if needed
           
           # Count distributions
           "PO"  = "poisson",         # Poisson
           "NB"  = "nbinomial",       # Negative Binomial
           "ZIP" = "zeroinflated.poisson",  # Zero-inflated Poisson
           "ZINB" = "zeroinflated.nbinomial",  # Zero-inflated NB (not native in INLA; needs latent model workaround)
           
           # Binary/Binomial
           "BI"  = "binomial",        # Binomial
           "BB"  = "betabinomial",    # Beta-binomial (if using with latent models or overdispersion)
           
           # Proportion or bounded continuous
           "BE"  = "beta",            # Beta distribution
           "LOGITNO" = "gaussian",    # Logit-Normal approx. (if transformed)
           
           # Fallback
           stop(paste("Family", family, "is not supported or requires custom implementation in INLA."))
    )
  }
  
  
  
  Result_Cancer =  switch(interface,
         
         "GAM" = GetTreeVariableGenesDynamics(mst =mst_grid,
                                              ExprsData = ExprsData,
                                              ClusterCol = ClusterCol,
                                              TemporalCol =TemporalCol, 
         ...
         "INLA" = GetTreeVariableGenesDynamics.INLA(mst =mst_grid,
                                               ExprsData = ExprsData,
                                               ClusterCol = ClusterCol,
                                               TemporalCol =TemporalCol, 
                                               ConfoundFrame=ConfoundFrame,
...
stopImplicitCluster() 
  ...
  GetcontrolNetwork <- GetcontrolNetwork( mst_grid,
                                          Data_sub,
                                          ControlDay = ControlDay,
                                          days)
  
  mst_grid_denoded = GetcontrolNetwork$controlNetwork
  Data_sub_reduced = GetcontrolNetwork$redueddatExpr
  Data_sub_reduced$days = ControlDay
  
  if(!is.null(ConfoundFrame)) ConfoundFrame_reduced = ConfoundFrame[GetcontrolNetwork$cell_id_in_control,] #Uncomment if adjusting for covariates
  
  Result_Cancer_control = switch(interface,
                                 
                                "GAM" = GetTreeVariableGenesDynamics(mst  =mst_grid_denoded,
                                                       ExprsData = Data_sub_reduced,
                                                       ClusterCol = ClusterCol,
                                                       TemporalCol =TemporalCol, 
                                                       ConfoundFrame=ConfoundFrame,
                                                       useWeight  = useWeight,
  ...
  "INLA" = GetTreeVariableGenesDynamics.INLA(mst =mst_grid_denoded,
                                             ExprsData = Data_sub_reduced,
                                             ClusterCol = ClusterCol,
                                             TemporalCol =TemporalCol, 
                                             ConfoundFrame=ConfoundFrame,
  ...
  return(list(Aux_result=Aux_result,
              Result_perturb = Result_Cancer,
              Result_control = Result_Cancer_control)
  )
}


GetNetwork <- function(datExpr,coords, sample_id,
                       thresholdGini=0.2,
                       k=10,
                       offset = c(.8, .8),
                       max.edge = c(3.9, 3.9),
  ...
  Data_sub =Coords = NULL
for(i in unique(sample_id)) {
  mini = sample_id==i
  datExpr_mini = datExpr[mini,]
  coords_mini = as.data.frame(scale(coords[mini,]))

  cat("CLustering for ",i,"\n")
  
  kmeans_result <- Hcluster(datExpr_mini,thresholdGini=thresholdGini,k=k,ClusterName="cluster")
  kmeans_result = as.data.frame(kmeans_result)
  
  cat("Constructing Mesh polygons","\n")
  
  meshId = getPolygonID(coords =coords_mini,
                        offset = offset,
                        max.edge = max.edge,
                        cutoff =cutoff,
                        Pron = Pron,
  ...
  UniqueCellID = paste0(meshId$cell.meshID,kmeans_result$cluster,i)
  
  cat("Collecting outputs","\n")
  
  Data_sub_mini =  datExpr_mini %>% as.data.frame() %>%
    bind_cols(clusterID.f = as.numeric(as.factor(UniqueCellID)) )
  Data_sub = rbind(Data_sub,Data_sub_mini)
  Coords = rbind(Coords,coords_mini)
}
  ...
  mst_grid = ClusterToTree(Centers = Centers2)
  
  # Number of nodes on tree
  m = vcount(mst_grid)
  
  return(list(Network =mst_grid, networkSize = m,
              Centers = Centers2,
              scaled_Centers = Centers2,
              UpdatedExprData = Data_sub,
              coords = Coords))
}

GetcontrolNetwork <- function(netwk =mst_grid,
                              datExpr,
                              ControlDay = 0,
                              days){
  ...
  mst_grid = netwk
  Data_sub = datExpr
  Data_sub_reduced <-  Data_sub[days == ControlDay,]
  cellNamesIncontrol <- Data_sub[days == ControlDay,]%>% rownames()
  # Get control network
  
  emptyNodes = which( !((1:m)%in%(Data_sub_reduced$clusterID.f %>% unique())))
  
  mst_grid_denoded = delete_vertices(mst_grid,emptyNodes)
  
  m.new = vcount(mst_grid_denoded)
  mst_grid_denoded = ReconectDisconectedNetwk(mst_grid_denoded)
  
  return( list(controlNetwork = mst_grid_denoded,
               redueddatExpr  = Data_sub_reduced,
               cell_id_in_control = cellNamesIncontrol))
}
...
GetTreeVariableGenesDynamics <- function(
  ...
  if(DownSample) cat("Downsampling = True: Downsampling by network node","\n")
  ...
  ko_param <- foreach(i=seq_len(length(names_gene)),.errorhandling = "pass",
    ...
    Hcluster <- function(DataTocluster,thresholdGini=0.2,k,ClusterName){
      ...
      cluster  <- genie::hclust2(objects=as.matrix(DataTocluster), thresholdGini=thresholdGini)
      #cluster  <- fastcluster::hclust(objects=as.matrix(DataTocluster), method = "ward.D2")
      clust    <- cutree(cluster, k = k)
    ...
    if(DownSample){
    #  cat("Downsampling = True: Downsampling by network node","\n")
      set.seed(23456)
    ...
    cat("Completed", (i/length(names_gene))*100,"%","\n" )
...
if(DownSample)  cat("Downsampling node by network nodes", "\n")
  ...
  ko_param <- foreach(i=seq_len(length(names_gene)),.errorhandling = "pass",
    ...
    sError = 1/m1$summary.hyperpar["Precision for s2",1]
      ...
      SNR[3,names_gene[i]] = sSpat
    SNR[2,names_gene[i]] = sError
    SNR[1,names_gene[i]] = sSpat/sError
